  ...
  var DesktopNavigation = /*#__PURE__*/function () {
    ...
    _createClass(DesktopNavigation, [{
      ...
      value: function _activateDropdown(event, target) {
        ...
        var isOpenAllLevels = target.classList.contains('openLevelLink');

        var menuToOpen = Dom.getSiblings(target, '[aria-hidden]')[0];
   
        var callback = function callback(menuToOpen) {
      ...
      if(isOpenAllLevels) {
            ...
            var menuToOpen1 = Dom.getSiblings(target, '[aria-hidden]')[0];
            var parentMenuEl = menuToOpen1.closest('.hasMenuItem');
            var siblingsMenu = parentMenuEl.parentNode.children;
            if(siblingsMenu.length) {
              siblingsMenu.forEach(function(item,index) {
                if(item.classList.contains('hasMenuItem')) {
                  item.classList.remove('is-dropdown-open');
                  item.querySelector('[data-type="menuitem"]').setAttribute('aria-expanded', 'false');
                  var menuToClose = item.querySelector('[aria-hidden]');
                  menuToClose.setAttribute('aria-hidden', 'true');
                  item.closest('[data-type="menu"]').classList.remove('nav-dropdown--glued'); 
                }
              });
            }
              callback(menuToOpen1);
            
            var menuToOpen2 = menuToOpen1.querySelector('.hasMenuItem [aria-hidden]');
            if(menuToOpen2) {
               callback(menuToOpen2);
            var menuToOpen3 = menuToOpen2.querySelector('.hasMenuItem [aria-hidden]');
              if(menuToOpen3) {
              	 callback(menuToOpen3);
              } 
            }
          } else {
            callback(menuToOpen);
...
//               console.log(siblingsMenu);
      ...
      value: function _deactivateDropdown(event, target) {
        ...
        var callback = function callback() {
          ...
          var menuToCloseArr = target.querySelectorAll('[aria-hidden]');
          
          for(var n = 0;n<menuToCloseArr.length;n++){
             menuToCloseArr[n].setAttribute('aria-hidden', 'true');
          }
          ...
          var isLastMenu = document.querySelector('.is-dropdown-open');
          if(isLastMenu == null) {
